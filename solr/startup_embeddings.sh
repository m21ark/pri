...
curl -X POST -H 'Content-type:application/json' \
    --data-binary "@./${SCHEMA_NAME}.json" \
    http://localhost:8983/solr/animals/schema

# Populate collection using a mapped path inside the container.
docker exec -it animal_exploration_solr bin/post -c animals /home/data/output_embeddings.json

#curl -X POST -H 'Content-type:application/json' \
#--data-binary "@./semanticSearch/em.json" \
#http://localhost:8983/solr/animals/update?commit=true
